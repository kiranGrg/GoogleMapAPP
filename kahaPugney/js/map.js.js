        ...
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            if(surf){ // to add a function while surfing on a map
                ...
                if(markerArray.length == 2){ //to triger a function to clean up the previous records of map after 2 markers are added on map
                    ...
                    pointArray.length = 0;
                    $('#serarch_result_lists').text(search_list);
                    search_list = '';
                }
            }
...
function setMarkerDiscription(jsonResult){
    ...
    search_list = search_list+" "+order+"= "+location_disp.name+","+location_disp.localArea;
...
function searchRequest(url, file_url, id){
   ...
   //alert(search.id+search.name+search.area+search.type+search.lat+search.lon+" and "+search.service_id+search.notice);
           ...
           if(id == 2){
                   $('#divNotice').empty();
                   $('<div class="notice_section"><center><font color="red">Notice:</font></center><br>'+search.notice+
                    '</div>').appendTo('#divNotice');
                       }
...
function reFreshField(){
    ...
    $('#lat').val('');
    $('#lon').val('');
    $('#p2_type option[value=""]').prop('selected', true);//to select the initial options
    $('#area').val('');
